...
def parsePDBstringInt(lines):
    ...
    for line in lines:
        if line[0:4] == "ATOM":
            ...
            domain = "%s" % (line[72:74].strip())
            if not domain in dPDB[chain].keys():
                dPDB[chain][domain] = {}
            ...
            if not curres in dPDB[chain][domain]["reslist"]:
                ...
                dPDB[chain][domain][curres] = {}
                dPDB[chain][domain][curres]["resname"] = string.strip(line[17:20])
                dPDB[chain][domain][curres]["atomlist"] = []
            atomtype = string.strip(line[12:16])
            dPDB[chain][domain][curres]["atomlist"].append(atomtype)
            dPDB[chain][domain][curres][atomtype] = {}
            # print "cures ", curres
            # print dPDB[chain][curres]

            dPDB[chain][domain][curres][atomtype]["x"] = float(line[30:38])
            dPDB[chain][domain][curres][atomtype]["y"] = float(line[38:46])
            dPDB[chain][domain][curres][atomtype]["z"] = float(line[46:54])
            dPDB[chain][domain][curres][atomtype]["domain"] = line[72:74].strip()
